...
function EditorMenuBar::onFileMenuItemSelect(%this, %itemId, %item)
{
   switch$(%item)
   {
      ...
      case "Exit...":
         if(ETerrainEditor.isMissionDirty || ETerrainEditor.isDirty || EWorldEditor.isDirty)
         {
            MessageBoxYesNo("Zone Modified", "Would you like to save changes to the current zone \"" @
            $Server::MissionFile @ "\" before exiting?", "EditorSaveMission();quit();", "quit();");
         }
         else
         {
            quit();
...
function Creator::init( %this ) 
{
   ...
   while( %file !$= "" ) 
   {
      ...
      for(%i=0; %i<%dirCount; %i++)
      {
         ...
         // if the group doesn't exist create it
         if ( !%sstaticId[%parent] )
            %sstaticId[%parent] = %this.addGroup( %parentId, getWord(%split, %i));
         %parentId = %sstaticId[%parent];
      }
      // Add the file to the group
    ...
    // ---------- Dynamic Shapes
   %base = %this.addGroup( 0, "Dynamic Shapes" );
   ...
   %Environment_Item[11] = "fxGrassReplicator";
   ...
   // Lighting Pack code block
   //-----------------------------------------------
   %Environment_Item[13] = "sgUniversalStaticLight";
...
function TextureInit()
{
   ...
   Texture_operation_menu.add("Place by File", 5);
...
//--------------------------------------
function Texture::setFile()
{
   getLoadFilename($TerraformerHeightfieldDir @ "/*.png", "Texture::doSetFile");
}

//--------------------------------------
function Texture::doSetFile(%name)
{
   file_name.setValue(%name);
   Texture::saveOperation();
   Texture::previewOperation();
}
...
function Texture_operation_menu::onSelect(%this, %id, %text)
{
   ...
   switch$ (%text)
   {
      ...
      case "Place by File":
         %id = Texture::addOperation("Place by File\ttab_FileMask\t" @ $nextTextureRegister++ @ "\t" @ %dreg @ "\tfile_name\t\tfileDistort\ttrue");
         Texture::setFile();
   }
...
function Texture::evalOperationData(%data, %row)
{
   ...
   switch$ (%label)
   {
      ...
      case "Place by File":
         terraformer.maskFile( %reg, getField(%data,5), getField(%data,7), %dreg );
   }
...
function tab_FW3D_bitmap::reset()
{
}
...
function TerraformerInit()
{
   ...
   Heightfield_options.add("Freeworld3D Bitmap",14);
...
function Heightfield_options::onSelect(%this, %_id, %text)
{
   ...
   switch$(%text)
   {
      ...
      case "Freeworld3D Bitmap":
         %id = Heightfield::add("Freeworld3D Bitmap\ttab_FW3D_Bitmap\tFW3D_bitmap_name\t");
         Heightfield::setFW3DBitmap();
   }
   ...
   if (Heightfield_operation.rowCount() >= 1)
   {
      switch$(%text)
      {
         ...
         case "Turbulence":
            %id = Heightfield::add("Turbulence\ttab_Turbulence\tturbulence_factor\t0.250\tturbulence_radius\t10\tturbulence_seed\t" @ terraformer.generateSeed());
...
function Heightfield::eval(%id)
{
   ...
   switch$(%label)
   {
      ...
      case "Turbulence":
         terraformer.turbulence( %row-1, %row, getField(%data,3), getField(%data,5), getField(%data,7) );
      ...
      case "Freeworld3D Bitmap":
         terraformer.loadGreyscale2(%row, getField(%data,3));
...
function Heightfield::apply(%id)
{
   ...
   Heightfield::refresh(%row);
   if (%label $= "Freeworld3D Bitmap")
      terraformer.setTerrain2(%row);
   else
...
//--------------------------------------
function Heightfield::setFW3DBitmap()
{
   getLoadFilename($TerraformerHeightfieldDir @ "/*.png", "Heightfield::doSetFW3DBitmap");
}

//--------------------------------------
function Heightfield::doSetFW3DBitmap(%name)
{
   FW3D_bitmap_name.setValue(%name);
   Heightfield::saveTab();
   Heightfield::preview($selectedOperation);
}
...
function Heightfield::hideTab()
{
   ...
   tab_FW3D_bitmap.setvisible(false);
