...
#
# Created:      12/9/04
# CVS-ID:       $Id: DebuggerService.py 6631 2005-12-19 19:00:59Z mhua $
...
class DebuggerService(Service.Service):
    ...
    #temp
    RUN_CREATEMASTERDB_ID = wx.NewId()
    RUN_CREATECHARACTERDB_ID = wx.NewId()
    ...
    def InstallControls(self, frame, menuBar = None, toolBar = None, statusBar = None, document = None):
        ...
        if not menuBar.FindItemById(DebuggerService.CLEAR_ALL_BREAKPOINTS):
            ...
            debuggerMenu.Append(DebuggerService.RUN_CLIENT_ID, _("&Run Client...\tCtrl+R"), _("Runs a file"))
            wx.EVT_MENU(frame, DebuggerService.RUN_CLIENT_ID, frame.ProcessEvent)
            wx.EVT_UPDATE_UI(frame, DebuggerService.RUN_CLIENT_ID, frame.ProcessUpdateUIEvent)

            debuggerMenu.AppendSeparator()

            debuggerMenu.Append(DebuggerService.RUN_MASTERSERVER_ID, _("&Run Master Server"), _("Runs the Master Server"))
            wx.EVT_MENU(frame, DebuggerService.RUN_MASTERSERVER_ID, frame.ProcessEvent)
            wx.EVT_UPDATE_UI(frame, DebuggerService.RUN_MASTERSERVER_ID, frame.ProcessUpdateUIEvent)

            debuggerMenu.Append(DebuggerService.RUN_CHARACTERSERVER_ID, _("&Run Character Server"), _("Runs the Character Server"))
            wx.EVT_MENU(frame, DebuggerService.RUN_CHARACTERSERVER_ID, frame.ProcessEvent)
            wx.EVT_UPDATE_UI(frame, DebuggerService.RUN_CHARACTERSERVER_ID, frame.ProcessUpdateUIEvent)

            debuggerMenu.Append(DebuggerService.RUN_WORLDDAEMON_ID, _("&Run World Daemon"), _("Runs the World Daemon"))
            wx.EVT_MENU(frame, DebuggerService.RUN_WORLDDAEMON_ID, frame.ProcessEvent)
            wx.EVT_UPDATE_UI(frame, DebuggerService.RUN_WORLDDAEMON_ID, frame.ProcessUpdateUIEvent)

            
            debuggerMenu.AppendSeparator()
                        
            debuggerMenu.Append(DebuggerService.RUN_CREATEMASTERDB_ID, _("&Create Master DB"), _("Creates Master DB File"))
            wx.EVT_MENU(frame, DebuggerService.RUN_CREATEMASTERDB_ID, frame.ProcessEvent)
            wx.EVT_UPDATE_UI(frame, DebuggerService.RUN_CREATEMASTERDB_ID, frame.ProcessUpdateUIEvent)

            debuggerMenu.Append(DebuggerService.RUN_CREATECHARACTERDB_ID, _("&Create Character DB"), _("Creates Character DB File"))
            wx.EVT_MENU(frame, DebuggerService.RUN_CREATECHARACTERDB_ID, frame.ProcessEvent)
            wx.EVT_UPDATE_UI(frame, DebuggerService.RUN_CREATECHARACTERDB_ID, frame.ProcessUpdateUIEvent)
        ...
        toolBar.AddTool(DebuggerService.RUN_CLIENT_ID, getRunningManBitmap(), shortHelpString = _("Run Client"), longHelpString = _("Run Client"))
...
#            return True
        elif an_id == DebuggerService.RUN_CLIENT_ID:
            self.OnRunClient(event)
            return True
        elif an_id == DebuggerService.RUN_MASTERSERVER_ID:
            self.OnRunMasterServer(event)
            return True
        elif an_id == DebuggerService.RUN_CHARACTERSERVER_ID:
            self.OnRunCharacterServer(event)
            return True
        elif an_id == DebuggerService.RUN_WORLDDAEMON_ID:
            self.OnRunWorldDaemon(event)
            return True
        
        elif an_id == DebuggerService.RUN_CREATEMASTERDB_ID:
            self.OnCreateMasterDB(event)
            return True

        elif an_id == DebuggerService.RUN_CREATECHARACTERDB_ID:
            self.OnCreateCharacterDB(event)
            return True
...
#            event.Enable(self.HasAnyFiles())
    ...
    def GetGameConfig(self):
        ...
        return os.path.basename(base)+".cfg"


    def OnCreateMasterDB(self, event):
        self.PromptToSaveFiles()
        gamecfg = self.GetGameConfig()
        self.ShowWindow(True)
        fileToRun = os.getcwd()+"/mud/masterserver/createdb.py"
        ...
        Service.ServiceView.bottomTab.SetSelection(count)
        page.Execute("gameconfig=%s"%gamecfg, os.getcwd(),os.environ)

    def OnCreateCharacterDB(self, event):
        self.PromptToSaveFiles()
        gamecfg = self.GetGameConfig()
        self.ShowWindow(True)
        fileToRun = os.getcwd()+"/mud/characterserver/createdb.py"
        shortFile = os.path.basename(fileToRun)
        page = RunCommandUI(Service.ServiceView.bottomTab, -1, fileToRun)
        count = Service.ServiceView.bottomTab.GetPageCount()
        Service.ServiceView.bottomTab.AddPage(page, "Running: " + shortFile)
    ...
    def OnRunGenesis(self, event):
        self.PromptToSaveFiles()
        gamecfg = self.GetGameConfig()
        self.ShowWindow(True)
        fileToRun = os.getcwd()+"/Genesis.py"
        shortFile = os.path.basename(fileToRun)
        page = RunCommandUI(Service.ServiceView.bottomTab, -1, fileToRun)
        count = Service.ServiceView.bottomTab.GetPageCount()
        Service.ServiceView.bottomTab.AddPage(page, "Running: " + shortFile)
    ...
    def OnRunClient(self, event,commandline=""):
        self.PromptToSaveFiles()
        gamecfg = self.GetGameConfig()
        self.ShowWindow(True)
        fileToRun = os.getcwd()+"/Client.pyw"
        shortFile = os.path.basename(fileToRun)
        page = RunCommandUI(Service.ServiceView.bottomTab, -1, fileToRun)
        count = Service.ServiceView.bottomTab.GetPageCount()
        Service.ServiceView.bottomTab.AddPage(page, "Running: " + shortFile)
        Service.ServiceView.bottomTab.SetSelection(count)
        cmdline = "gameconfig=%s"%gamecfg
        if commandline:
            cmdline+=" "+commandline
        page.Execute(cmdline, os.getcwd(),os.environ)

    def OnRunMasterServer(self, event):
        self.PromptToSaveFiles()
        gamecfg = self.GetGameConfig()
        self.ShowWindow(True)
        fileToRun = os.getcwd()+"/mud/masterserver/main.py"
        shortFile = os.path.basename(fileToRun)
        page = RunCommandUI(Service.ServiceView.bottomTab, -1, fileToRun)
        count = Service.ServiceView.bottomTab.GetPageCount()
        Service.ServiceView.bottomTab.AddPage(page, "Running: " + shortFile)
    ...
    def OnRunCharacterServer(self, event):
        self.PromptToSaveFiles()
        gamecfg = self.GetGameConfig()
        self.ShowWindow(True)
        fileToRun = os.getcwd()+"/mud/characterserver/server.py"
        shortFile = os.path.basename(fileToRun)
        page = RunCommandUI(Service.ServiceView.bottomTab, -1, fileToRun)
        count = Service.ServiceView.bottomTab.GetPageCount()
        Service.ServiceView.bottomTab.AddPage(page, "Running: " + shortFile)
    ...
    def OnRunWorldDaemon(self, event):
        if _WINDOWS:
            print "run world daemon"
            
            cmd  = sys.executable
            path = os.getcwd()
            args = '%s %s/WorldDaemon.py '%(cmd,path)
            args += "-worldname=Premium_MMORPG -publicname=starter -password=mmo gameconfig=%s"%self.GetGameConfig()
            os.spawnl(os.P_DETACH,cmd,args)
        else:
            self.PromptToSaveFiles()
            gamecfg = self.GetGameConfig()
            self.ShowWindow(True)
            fileToRun = os.getcwd()+"/WorldDaemon.py"
            shortFile = os.path.basename(fileToRun)
            page = RunCommandUI(Service.ServiceView.bottomTab, -1, fileToRun)
            count = Service.ServiceView.bottomTab.GetPageCount()
            Service.ServiceView.bottomTab.AddPage(page, "Running: " + shortFile)
            Service.ServiceView.bottomTab.SetSelection(count)
            page.Execute("gameconfig=%s -worldname=Premium_MMORPG -publicname=starter -password=mmo"%gamecfg, os.getcwd(),os.environ)
